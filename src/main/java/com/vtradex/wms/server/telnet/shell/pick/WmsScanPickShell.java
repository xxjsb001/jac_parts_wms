...
public class WmsScanPickShell extends Thorn4BaseShell{
	...
	protected void mainProcess(Connection connection) throws BreakException,
		...
		String pickNo = (String) this.getParentValue(PICK_CODE);
		String container = (String) this.getParentValue(CONTAINER);
		String location = (String) this.getParentValue(LOC_CODE);
...
//			Map<String,String> result = pickRFManager.findLoc(location);
//			messge = result.get(ERROR_MESG);
//			if(!StringUtils.isEmpty(messge)){ 
//				this.setStatusMessage(messge);
//			}
			...
			this.put(PICK_CODE, pickNo);
			this.put(CONTAINER, container);
			this.put(LOC_CODE, location);
			this.forward(WmsScanPickShell.PAGE_ID, messge);
		}else if(StringUtils.isEmpty(itemCode)){
			output("pick.location",location);
...
//			Map<String,String> result = pickRFManager.findMoveDetail(pickNo, itemCode);
//			messge = result.get(ERROR_MESG);
//			if(!StringUtils.isEmpty(messge)){ 
//				this.setStatusMessage(messge);
//			}
			...
			itemCode = (String) this.getParentValue(ITEM_CODE);
			...
			if(!StringUtils.isEmpty(messge)){
				if(messge.equals(ERROR_ITEM_NULL) //物料明细不存在
						|| messge.equals(ERROR_ITEM_FINISHED)//物料拣货完成
						|| messge.equals(ERROR_LOSS)//系统库存不足
						|| messge.equals(ERROR_SHIP_LOC_NULL)){//备货库位不存在
					this.put(PICK_CODE, pickNo);
					this.put(CONTAINER, container);
					this.put(LOC_CODE, location);
					this.forward(WmsScanPickShell.PAGE_ID, messge);
				}else if(messge.equals(ERROR_CONTAINER_NULL)){//容器不存在或失效
					this.put(PICK_CODE, pickNo);
					this.forward(WmsScanPickShell.PAGE_ID, messge);
				}else if(messge.equals(ERROR_LOC_NULL)){//库位号不存在
					this.put(PICK_CODE, pickNo);
					this.put(CONTAINER, container);
					this.forward(WmsScanPickShell.PAGE_ID, messge);
				}else if(messge.equals(ERROR_PIC_MOVE)
						|| messge.equals(ERROR_PIC_MOVE_INV)){//失败!拣货量超出可用量
					this.put(PICK_CODE, pickNo);
					this.put(CONTAINER, container);
					this.put(LOC_CODE, location);
					this.put(ITEM_CODE, itemCode);
