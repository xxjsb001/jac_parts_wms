...
/**补货移位-入口*/
public class WmsReplenishmentMoveMoveShell extends Thorn4BaseShell {
	public static final String PAGE_ID = "wmsReplenishmentMoveMoveShell";
	private final WmsMoveRFManager rfWmsMoveManager;

	public WmsReplenishmentMoveMoveShell(WmsMoveRFManager rfWmsMoveManager) {
		this.rfWmsMoveManager = rfWmsMoveManager;
	}
	protected void mainProcess(Connection connection) throws BreakException,
			ContinueException, IOException, Exception {
		List<Object[]> moveDocs = rfWmsMoveManager.findUnActiveReplenishMove();
		String messge = "补货移位明细";
		if(moveDocs.size()>0){
			...
			rfWmsMoveManager.activePickByRf(moveCode);
			...
			this.getTextField("无补货移位任务(..返回主菜单;...退出登录)");
			this.forward(WmsReplenishmentMoveMoveShell.PAGE_ID);
		}
	}
