...
/**移位上架 */
public class WmsScanMoveUpShell extends Thorn4BaseShell{
	public static final String PAGE_ID = "wmsScanMoveUpShell";
	String messge = "操作成功,请继续"+MyUtils.enter;
	
	
	private final WmsPickRFManager pickRFManager;
	public WmsScanMoveUpShell(WmsPickRFManager pickRFManager) {
	...
	protected void mainProcess(Connection connection) throws BreakException,
		...
		if(StringUtils.isEmpty(moveNo)){
			...
			this.forward(WmsScanMoveUpShell.PAGE_ID, messge);
			...
			Map<String,String> result = pickRFManager.singleMoveUpQty(moveNo, location, itemCode,picQuantity,WmsWarehouseHolder.getWmsWarehouse().getId());
			messge = result.get(WmsScanPickShellMessage.ERROR_MESG);
			if(!StringUtils.isEmpty(messge)){
