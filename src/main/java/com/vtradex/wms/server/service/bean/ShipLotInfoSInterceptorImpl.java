...
public class ShipLotInfoSInterceptorImpl implements AfterReturnIntercepter {
	private WmsItemManager itemManager;

	public ShipLotInfoSInterceptorImpl(WmsItemManager itemManager) {
		super();
	...
	public Object afterReturn(Object object) {
		if(object instanceof TableConfig){
			...
			for (RowData rw : list) {
				...
				List<Boolean> results = itemManager.getShipLotRuleTackers(itemId);
